...
{
    [TestFixture]
    public class BookingSearchManagerTests
    {
        private readonly CultureInfo _cultureInfo = new CultureInfo("nl-NL");
        private readonly BookingSearchManager _bookingSearchManager;

        public BookingSearchManagerTests()
        {
            var bookingsRepositoryMock = new Mock<IRepository<Booking>>();

            bookingsRepositoryMock.Setup(m => m.GetAll()).Returns(GenerateBookings());

            _bookingSearchManager = new BookingSearchManager(bookingsRepositoryMock.Object);
        }

        [Test]
        public void Test_should_return_passengers_for_exact_flight()
        {
            var existingFlightNumber = "PZ696";

            var passemgersForFlight = GenerateBookings()
                .Where(booking => booking.Flight.Number == existingFlightNumber)
                .SelectMany(booking => booking.Passengers)
                .Distinct()
                .ToArray();

            var passengers = _bookingSearchManager.GetPassengersForFlight(existingFlightNumber).ToArray();

            var expectedPassengerIds = passemgersForFlight.Select(passenger => passenger.Id);
            var actualPassengerIds = passengers.Select(passenger => passenger.Id);

            Assert.NotNull(passengers);
            Assert.IsNotEmpty(passengers);
            Assert.AreEqual(passemgersForFlight.Length, passengers.Length);
            Assert.That(expectedPassengerIds, Is.EquivalentTo(actualPassengerIds));
        }

        [Test]
        public void Test_should_throw_DomainObjectNotFoundException_for_flight_not_in_the_system()
        {
            var notPresentedFlightNumber = "I am not presented";

            Assert.Throws<DomainObjectNotFoundException>(() => _bookingSearchManager.GetPassengersForFlight(notPresentedFlightNumber));
        }
